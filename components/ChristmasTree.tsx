...
export const ChristmasTree: React.FC<ChristmasTreeProps> = ({ 
  ...
  const geometry = useMemo(() => {
    ...
    const treeParticles = 90000; // Increased density for a lush look
    ...
    for (let i = 0; i < treeParticles; i++) {
      ...
      // Ensures uniform density on the cone surface (prevents clumping at top)
      ...
      if (distFromSurface > 0.85 && Math.random() > 0.97) {
         ...
         let shadow = 1.0;
         if (hNorm < 0.85) {
             // Lightened shadows from 0.4 to 0.6 for brighter core
             const innerDarkness = rRatio < 0.4 ? 0.6 : 1.0;
             shadow = THREE.MathUtils.lerp(innerDarkness, 1.0, hNorm * 1.5);
         }

         const depth = distFromSurface; 
         // Increased base ambient light from 0.2 to 0.5
         const ambient = 0.5 + (hNorm * 0.5); 
         // Increased base diffuse light from 0.2 to 0.5
         const diffuse = 0.5 + depth * 0.8;
         let brightness = Math.max(ambient, diffuse);

         // APEX HANDLING
         if (hNorm > 0.85) {
             brightness = 2.0 + (hNorm - 0.85) * 8.0;
             shadow = 1.0;
         }

         // Global Brightness Boost (1.6x)
         color.copy(baseTreeColor).multiplyScalar(brightness * 1.6).multiplyScalar(shadow);
         
         if (hNorm > 0.75) {
             const proximityToStar = (hNorm - 0.75) / 0.25; 
             const glow = Math.pow(Math.max(0, proximityToStar), 2.5);
             const glowColor = starLightColor.clone().multiplyScalar(glow * 1.2);
             color.add(glowColor);
         }
      }
