...
export const Gifts: React.FC<GiftsProps> = ({ isVisible = true }) => {
  ...
  const handleBeforeCompile = (shader: any) => {
    ...
    shader.uniforms.uGlitterColor = { value: new THREE.Color('#ffcc00') }; 
    ...
    // Apply glitter in the final color mix
    shader.fragmentShader = shader.fragmentShader.replace(
      '#include <dithering_fragment>',
      `#include <dithering_fragment>
       
       // Procedural Glitter Logic
       vec2 gUV = vGlitterUv * 50.0; // High frequency for fine particles
       float gNoise = gRand(floor(gUV) + floor(uTime * 6.0)); // Animated noise
       
       // Sparse threshold (1.5% coverage)
       float gSparkle = step(0.985, gNoise);
       
       // Pulse over time
       float gPulse = 0.5 + 0.5 * sin(uTime * 2.0 + vGlitterUv.x * 20.0);
       
       // Additive sparkle glow
       gl_FragColor.rgb += uGlitterColor * gSparkle * gPulse * 1.5;
      `
    );
  ...
  useFrame((state) => {
     ...
     // Update all registered shaders with current time
