      ...
      <div className={`absolute left-16 top-1/2 transform -translate-y-1/2 w-80 pointer-events-auto transition-all duration-500 ease-in-out ${isVisible ? 'translate-x-0 opacity-100' : '-translate-x-[120%] opacity-0'}`}>
        <div className="bg-[#031f18]/80 backdrop-blur-md border border-[#d4af37]/30 rounded-2xl p-6 shadow-2xl flex flex-col gap-4 hover:border-[#d4af37]/60 max-h-[80vh] overflow-y-auto custom-scrollbar">
          ...
          {config.photoUrls.length > 0 && (
            <div className="grid grid-cols-4 gap-2 mt-2 p-2 bg-black/20 rounded-lg">
                {config.photoUrls.map((url, idx) => (
                    <div key={idx} className="relative aspect-square rounded-md overflow-hidden border border-gray-600 group/thumb">
                        <img src={url} alt={`Uploaded ${idx}`} className="w-full h-full object-cover" />
                        <button 
                            onClick={() => removePhoto(idx)}
                            className="absolute inset-0 bg-black/60 opacity-0 group-hover/thumb:opacity-100 flex items-center justify-center text-white transition-opacity text-xs hover:text-red-400"
                        >
